...
概念只作为 WebUI 的阅读辅助：`/tasks/{id}/concepts` 返回 JSON，前端点「加载概念」后自行渲染，**不写回成稿**，也不进入 `/download` 与连接器导出的正文——前端只是在阅读页正文里原地把每个概念词的首次出现变成可点的维基百科链接（`app.js` 的 `linkifyManuscriptConcepts`，纯 DOM 层面），侧栏列表同时展示完整清单。
