...
- `concepts_to_markdown(concepts)`：渲染成「延伸阅读」列表。
- `strip_concepts_section(text)` / `upsert_concepts_section(text, concepts)`：把「延伸阅读」写进成稿——已有就替换、没有就插在末尾 `*Generated by Read Podcast*` 署名块之前，因此重复加载不会堆出多份。`/tasks/{id}/concepts` 算完即写回文件，这样 `/download`（直接吐磁盘文件）下载到的成稿就带着延伸阅读，而下载本身仍是零等待。抽取前也会先用 `strip_concepts_section` 剔掉上一轮结果，避免把自己生成的链接列表当正文喂回模型。
  - 注意：概念缓存以成稿 `mtime` 为键，而写回会改 `mtime`，所以写回后必须用新的 `mtime` 重算缓存键，否则这份代价高昂的抽取会被每次加载反复重跑。
