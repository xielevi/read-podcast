...
| GET | `/integrations` | Google/飞书 OAuth 应用与账号连接状态（只返回布尔状态） |
| PUT | `/integrations/{provider}/app` | 保存 Google Client 或飞书 App 凭据到本机 `secrets.env` |
| POST | `/integrations/{provider}/authorize` | 生成同源回调、一次性 state 与第三方授权地址 |
| GET | `/integrations/{provider}/callback` | 校验 state、交换并保存刷新令牌，通知登录窗口 |
...
**云文档账号（`/integrations`）** 使用服务端 Authorization Code Flow。Google 请求 `drive.file` 与离线访问；飞书交换并轮换 `user_access_token`/`refresh_token`。`redirect_uri` 必须与发起页面同源并严格匹配当前 provider 的回调路径，state 十分钟过期且只能消费一次。浏览器只接收成功/失败状态，任何 Client Secret、访问令牌或刷新令牌都不会通过状态接口或回调页返回。
