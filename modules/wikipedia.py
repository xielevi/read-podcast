...
CONCEPTS_HEADING = "## 延伸阅读（维基百科）"

# 成稿末尾由 Formatter 固定加上的署名块，延伸阅读要插在它前面而不是后面。
_FOOTER_RE = re.compile(r"\n*---\n\*Generated by Read Podcast\*\s*\n?\s*$")
# 段落边界：下一个一级/二级标题，或成稿末尾署名块的分隔线。
_SECTION_BOUNDARY_RE = re.compile(r"^(#{1,2} |---\s*$)")


def concepts_to_markdown(concepts: list[dict], heading: str = CONCEPTS_HEADING) -> str:
    """把概念列表渲染成 Markdown，供附在正文后。"""
    if not concepts:
        return ""
    lines = [heading, ""]
    for concept in concepts:
...
def strip_concepts_section(text: str, heading: str = CONCEPTS_HEADING) -> str:
    """删掉正文里已有的「延伸阅读」段落，没有则原样返回。

    重复加载概念时先删后插，成稿里就只会有一份，不会越堆越多；抽取概念前也用它
    把上一轮的结果剔掉，避免把自己生成的链接列表又当成正文喂回给模型。
    """
    if not text:
        return text or ""
...
def upsert_concepts_section(
    text: str,
    concepts: list[dict],
    heading: str = CONCEPTS_HEADING,
) -> str:
    """把「延伸阅读」写进正文：已有就替换，没有就插在末尾署名块之前。"""
    body = strip_concepts_section(text, heading)
    section = concepts_to_markdown(concepts, heading)
    if not section:
        return body

    footer = _FOOTER_RE.search(body)
    if footer:
        head = body[: footer.start()].rstrip("\n")
        return f"{head}\n\n{section}\n\n{body[footer.start():].lstrip('\n')}"
