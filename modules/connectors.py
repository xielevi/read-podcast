...
def _env_value(name: str) -> str:
    return os.environ.get(str(name or "").strip(), "").strip()
...
def _required_env(connector: dict) -> list[str] | None:
    ...
    fmt = str(connector.get("format", "")).strip()
    ...
    names: list[str] = []
    for key in keys:
        name = str(connector.get(key, "")).strip()
        if not name:
            return None
        names.append(name)
    return names
...
def _connector_configured(connector: dict) -> bool:
    ...
    if names is None:
        return False
    if not all(_env_value(name) for name in names):
        return False
    # 云文档还需要指向具体目标
    fmt = connector.get("format")
    if fmt == "notion":
        return bool(str(connector.get("database_id", "")).strip() or str(connector.get("page_id", "")).strip())
    return True
...
def _send_webhook(connector: dict, doc: dict, *, timeout: int) -> dict:
    name = str(connector.get("name", "")).strip()
    fmt = str(connector.get("format", "markdown")).strip()
    url = _env_value(connector.get("url_env", ""))
    if not str(connector.get("url_env", "")).strip():
...
def _send_notion(connector: dict, doc: dict, *, timeout: int) -> dict:
    name = str(connector.get("name", "")).strip()
    token = _env_value(connector.get("token_env", ""))
    if not str(connector.get("token_env", "")).strip():
        raise ConnectorError(f"连接器「{name}」未配置 token_env。")
    if not token:
        raise ConnectorError(f"连接器「{name}」的 Notion token 未设置（环境变量为空）。")

    database_id = str(connector.get("database_id", "")).strip()
    page_id = str(connector.get("page_id", "")).strip()
    ...
    if database_id:
        title_property = str(connector.get("title_property", "") or "Name").strip() or "Name"
        parent = {"database_id": database_id}
        properties = {title_property: {"title": _notion_rich_text(title)}}
    else:
        parent = {"page_id": page_id}
        properties = {"title": {"title": _notion_rich_text(title)}}

    base = str(connector.get("base_url", "") or "https://api.notion.com").rstrip("/")
    headers = {
        "Authorization": f"Bearer {token}",
        "Notion-Version": str(connector.get("notion_version", "") or NOTION_VERSION),
        "Content-Type": "application/json",
    }
    payload = {"parent": parent, "properties": properties, "children": children}
    response = _post_json(
        f"{base}/v1/pages", json_body=payload, headers=headers, timeout=timeout, label=f"连接器「{name}」"
    )
    ...
    document_url = ""
    try:
        document_url = str(response.json().get("url", "") or "")
    except Exception:
        pass
    return {
        "connector": name,
...
def _feishu_base(connector: dict) -> str:
    return str(connector.get("base_url", "") or FEISHU_DEFAULT_BASE).rstrip("/")


def _feishu_tenant_token(connector: dict, *, timeout: int) -> str:
    name = str(connector.get("name", "")).strip()
    app_id = _env_value(connector.get("app_id_env", ""))
    app_secret = _env_value(connector.get("app_secret_env", ""))
    if not str(connector.get("app_id_env", "")).strip() or not str(connector.get("app_secret_env", "")).strip():
...
def _send_feishu_doc(connector: dict, doc: dict, *, timeout: int) -> dict:
    ...
    folder_token = str(connector.get("folder_token", "")).strip()
    if folder_token:
...
def test_connector(connector: dict, *, timeout: int = 15) -> dict:
    ...
    # feishu-doc：换取一次 tenant_access_token 即可验证应用凭据。
